...
#include "AbilitySystem/Tasks/AsyncTask_AttributeChanged.h"
	...
	UAbilitySystemComponent* AbilitySystemComponent, FGameplayAttribute Attribute)
{
	...
	AddUObject(WaitForAttributeChangedTask, &UAsyncTask_AttributeChanged::AttributeChanged);
...
void UAsyncTask_AttributeChanged::AttributeChanged(const FOnAttributeChangeData& Data)
{
	...
	if (Data.GEModData != nullptr)
	{
		...
		if (!Instigator)
		{
			Instigator = EffectContext.GetEffectCauser();
		}
		
		
	}

	OnAttributeChanged.Broadcast(Data.Attribute, Instigator,Data.NewValue, Data.OldValue);
}
