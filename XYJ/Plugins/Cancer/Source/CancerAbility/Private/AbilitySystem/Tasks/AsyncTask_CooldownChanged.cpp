...
#include "AbilitySystem/Tasks/AsyncTask_CooldownChanged.h"

#include "ActiveGameplayEffectHandle.h"


UAsyncTask_CooldownChanged* UAsyncTask_CooldownChanged::ListenForCooldownChange(
	UAbilitySystemComponent* AbilitySystemComponent, FGameplayTagContainer CooldownTags, bool UseServerCooldown)
{
	...
	AbilitySystemComponent->OnActiveGameplayEffectAddedDelegateToSelf.AddUObject(
		ListenForCooldownChange, &UAsyncTask_CooldownChanged::OnActiveGameplayEffectAddedCallback);
	...
	{
		AbilitySystemComponent->RegisterGameplayTagEvent(CooldownTag,
			EGameplayTagEventType::NewOrRemoved).AddUObject(ListenForCooldownChange, &UAsyncTask_CooldownChanged::CooldownTagChanged);
	}
...
void UAsyncTask_CooldownChanged::OnActiveGameplayEffectAddedCallback(UAbilitySystemComponent* Target,
...
void UAsyncTask_CooldownChanged::CooldownTagChanged(const FGameplayTag CooldownTag, int32 NewCount)
{
	if (NewCount == 0)
	{
...
bool UAsyncTask_CooldownChanged::GetCooldownRemainingForTag(FGameplayTagContainer InCooldownTags, float& TimeRemaining,
