...
#include "FreeBack/ContextEffects/AnimNotify_CancerContextEffects.h"
...
#include "FreeBack/ContextEffects/CancerContextEffectsInterface.h"
#include "FreeBack/ContextEffects/CancerContextEffectsLibrary.h"
#include "FreeBack/ContextEffects/CancerContextEffectsSubsystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimNotify_CancerContextEffects)



UAnimNotify_CancerContextEffects::UAnimNotify_CancerContextEffects()
{
}

void UAnimNotify_CancerContextEffects::PostLoad()
{
	Super::PostLoad();
}

#if WITH_EDITOR
void UAnimNotify_CancerContextEffects::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
}
#endif

FString UAnimNotify_CancerContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_CancerContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			TArray<UObject*> CancerContextEffectImplementingObjects;

			// Determine if the Owning Actor is one of the Objects that implements the Context Effects Interface
			if (OwningActor->Implements<UCancerContextEffectsInterface>())
			{
				// If so, add it to the Array
				CancerContextEffectImplementingObjects.Add(OwningActor);
			}
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<UCancerContextEffectsInterface>())
					{
						CancerContextEffectImplementingObjects.Add(Component);
					}
				}
			}

			// Cycle through all objects implementing the Context Effect Interface
			for (UObject* CancerContextEffectImplementingObject : CancerContextEffectImplementingObjects)
			{
				if (CancerContextEffectImplementingObject)
				{
					// If the object is still valid, Execute the AnimMotionEffect Event on it, passing in relevant data
					ICancerContextEffectsInterface::Execute_AnimMotionEffect(CancerContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					{
						// Check if it is in fact a UCancerContextEffectLibrary type
						if (UCancerContextEffectsLibrary* EffectLibrary = Cast<UCancerContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_CancerContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_CancerContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
	FCancerContextEffectAnimNotifyVFXSettings VFXPropertiesIn, FCancerContextEffectAnimNotifyAudioSettings AudioPropertiesIn,
	bool bAttachedIn, FName SocketNameIn, bool bPerformTraceIn, FCancerContextEffectAnimNotifyTraceSettings TracePropertiesIn)
{
	Effect = EffectIn;
