...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CancerContextEffectsSubsystem)
...
void UCancerContextEffectsSubsystem::SpawnContextEffects(
	...
	, float AudioPitch)
{
	...
	if (TObjectPtr<UCancerContextEffectsSet>* EffectsLibrariesSetPtr = ActiveActorEffectsMap.Find(SpawningActor))
	{
		// Validate the pointers from the Map Find
		if (UCancerContextEffectsSet* EffectsLibraries = *EffectsLibrariesSetPtr)
		{
			...
			for (UCancerContextEffectsLibrary* EffectLibrary : EffectsLibraries->CancerContextEffectsLibraries)
			{
...
bool UCancerContextEffectsSubsystem::GetContextFromSurfaceType(
	TEnumAsByte<EPhysicalSurface> PhysicalSurface, FGameplayTag& Context)
{
	// Get Project Settings
	if (const UCancerContextEffectsSettings* ProjectSettings = GetDefault<UCancerContextEffectsSettings>())
	{
...
void UCancerContextEffectsSubsystem::LoadAndAddContextEffectsLibraries(AActor* OwningActor,
	TSet<TSoftObjectPtr<UCancerContextEffectsLibrary>> ContextEffectsLibraries)
{
	...
	UCancerContextEffectsSet* EffectsLibrariesSet = NewObject<UCancerContextEffectsSet>(this);

	// Cycle through Libraries getting Soft Obj Refs
	for (const TSoftObjectPtr<UCancerContextEffectsLibrary>& ContextEffectSoftObj : ContextEffectsLibraries)
	{
		// Load Library Assets from Soft Obj refs
		// TODO Support Async Loading of Asset Data
		if (UCancerContextEffectsLibrary* EffectsLibrary = ContextEffectSoftObj.LoadSynchronous())
		{
			// Call load on valid Libraries
			EffectsLibrary->LoadEffects();

			// Add new library to Set
			EffectsLibrariesSet->CancerContextEffectsLibraries.Add(EffectsLibrary);
		}
	}
...
void UCancerContextEffectsSubsystem::UnloadAndRemoveContextEffectsLibraries(AActor* OwningActor)
{
