...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CancerContextEffectsLibrary)


void UCancerContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void UCancerContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState UCancerContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void UCancerContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FCancerContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<UCancerActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FCancerContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			UCancerActiveContextEffects* NewActiveContextEffects = NewObject<UCancerActiveContextEffects>(this);
	...
	this->CancerContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void UCancerContextEffectsLibrary::CancerContextEffectLibraryLoadingComplete(
	TArray<UCancerActiveContextEffects*> CancerActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(CancerActiveContextEffects);
}
