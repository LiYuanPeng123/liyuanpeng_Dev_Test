...
bool FIndicatorProjection::Project(const UIndicatorDescriptor& IndicatorDescriptor, const FSceneViewProjectionData& InProjectionData, const FVector2f& ScreenSize, FVector& OutScreenPositionWithDepth)
{
	if (USceneComponent* Component = IndicatorDescriptor.GetSceneComponent())
	{
		...
		{
			FName SocketName = IndicatorDescriptor.GetComponentSocketName();
			WorldLocation = Component->GetSocketTransform(SocketName).GetLocation();
		}
		else
		{
...
{
	bVisible = InVisible;
	if (auto Widget = GetIndicatorWidget())
	{
		if (Widget->Implements<UIndicatorWidgetInterface>())
		{
			IIndicatorWidgetInterface::Execute_UpdateIndicatorVisible(Widget
			                                                          , Widget, this);
		}
	}
}

void UIndicatorDescriptor::SetInteraction(bool bValue)
{
	bInteraction = bValue;
	if (auto Widget = GetIndicatorWidget())
	{
		if (Widget->Implements<UIndicatorWidgetInterface>())
		{
			IIndicatorWidgetInterface::Execute_IndicatorInteraction(Widget
			                                                        , Widget, this);
		}
	}
}

void UIndicatorDescriptor::SetIndicatorManagerComponent(UCancerIndicatorManagerComponent* InManager)
{
...
{
	if (UCancerIndicatorManagerComponent* Manager = ManagerPtr.Get())
	{
