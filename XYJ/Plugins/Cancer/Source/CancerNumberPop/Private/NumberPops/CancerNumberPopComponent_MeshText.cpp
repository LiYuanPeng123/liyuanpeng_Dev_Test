#include "NumberPops/CancerNumberPopComponent_MeshText.h"

#include "NumberPops/CancerDamagePopStyle.h"

UCancerNumberPopComponent_MeshText::UCancerNumberPopComponent_MeshText(
	const FObjectInitializer& ObjectInitializer): Super(ObjectInitializer)
{
	...
	PositionParameterNames = {
		TEXT("0a"), TEXT("1a"), TEXT("2a"), TEXT("3a"), TEXT("4a"), TEXT("5a"), TEXT("6a"), TEXT("7a"), TEXT("8a")
	};
	ScaleRotationAngleParameterNames = {
		TEXT("0b"), TEXT("1b"), TEXT("2b"), TEXT("3b"), TEXT("4b"), TEXT("5b"), TEXT("6b"), TEXT("7b"), TEXT("8b")
	};
	DurationParameterNames = {
		TEXT("0c"), TEXT("1c"), TEXT("2c"), TEXT("3c"), TEXT("4c"), TEXT("5c"), TEXT("6c"), TEXT("7c"), TEXT("8c")
	};
...
void UCancerNumberPopComponent_MeshText::AddNumberPop(const FCancerNumberPopRequest& NewRequest)
{
	...
	// Grab a component from the pool for this number or create one
	{
		...
		{
			LocalWorld->GetTimerManager().SetTimer(ReleaseTimerHandle, this, &ThisClass::ReleaseNextComponents,
			                                       ComponentLifespan);
		}
	}
	...
	if (APlayerController* PC = GetController<APlayerController>())
	{
		...
		{
			CameraTransform = FTransform(PlayerCameraManager->GetCameraRotation(),
	...
	PreparedNumberInfo.StaticMeshComponent->
	                   SetWorldTransform(FTransform(CameraTransform.GetRotation(), NumberLocation));
...
void UCancerNumberPopComponent_MeshText::SetMaterialParameters(const FCancerNumberPopRequest& Request,
                                                               FTempNumberPopInfo& NewDamageNumberInfo,
                                                               const FTransform& CameraTransform,
                                                               const FVector& NumberLocation)
{
...
FLinearColor UCancerNumberPopComponent_MeshText::DetermineColor(const FCancerNumberPopRequest& Request) const
{
	for (UCancerDamagePopStyle* Style : Styles)
	{
...
UStaticMesh* UCancerNumberPopComponent_MeshText::DetermineStaticMesh(
	const FCancerNumberPopRequest& Request) const
{
	for (UCancerDamagePopStyle* Style : Styles)
	{
...
void UCancerNumberPopComponent_MeshText::ReleaseNextComponents()
{
	UWorld* LocalWorld = GetWorld();
	check(LocalWorld);
