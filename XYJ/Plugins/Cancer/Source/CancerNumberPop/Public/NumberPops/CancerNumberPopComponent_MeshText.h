...
#include "CancerNumberPopComponent_MeshText.generated.h"

class UCancerDamagePopStyle;
...
class CANCERNUMBERPOP_API UCancerNumberPopComponent_MeshText : public UCancerNumberPopComponent
{
	GENERATED_BODY()

public:
	UCancerNumberPopComponent_MeshText(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
protected:
	void SetMaterialParameters(const FCancerNumberPopRequest& Request, FTempNumberPopInfo& NewDamageNumberInfo,
	                           const FTransform& CameraTransform, const FVector& NumberLocation);

	FLinearColor DetermineColor(const FCancerNumberPopRequest& Request) const;
	UStaticMesh* DetermineStaticMesh(const FCancerNumberPopRequest& Request) const;
	...
	TArray<TObjectPtr<UCancerDamagePopStyle>> Styles;
