...
struct FCancerInputHandlerHelpers
{
    /**
     ...
     */
    static bool HasTags(const UCancerInputManagerComponent* Manager, const FGameplayTagQuery& Query)
    {
        if (Query.IsEmpty())
        {
    ...
    static int32 SendGameplayEvent(const UCancerInputManagerComponent* Manager, const FGameplayTag& GameplayEventTag, const FInputActionValue& Value, const UInputAction* InputAction, const FString Context = TEXT("Local Execution"))
    {
        int32 Activations = 0;
        const TObjectPtr<UAbilitySystemComponent> AbilitySystemComponent = Manager->GetAbilitySystemComponent();
            ...
            ensureMsgf(GameplayEventTag.IsValid(), TEXT("The Gameplay Event Tag must be valid.")))
        {
            ...
            UE_LOG(CancerInputLog, Verbose, TEXT("[%s] Action %s triggered event %s with %d activations. Context: %s"),
     ...
     */
    static void InterruptAbilityByClass(const UCancerInputManagerComponent* Manager, const UInputAction* InputAction, const TSubclassOf<UGameplayAbility>& AbilityClass)
    {
        const TObjectPtr<UAbilitySystemComponent> AbilitySystemComponent = Manager->GetAbilitySystemComponent();
        if (ensure(IsValid(AbilitySystemComponent)))
        {
            ...
            {
                UE_LOG(CancerInputLog, Verbose, TEXT("[%s] Action %s is interrupting ability with class %s."),
     ...
     */
    static void InterruptAbilityByTags(const UCancerInputManagerComponent* Manager, const UInputAction* InputAction, const FGameplayTagContainer& AbilityTags, const FGameplayTagContainer& CancelFilterTags)
    {
        const TObjectPtr<UAbilitySystemComponent> AbilitySystemComponent = Manager->GetAbilitySystemComponent();
        if (ensure(IsValid(AbilitySystemComponent)))
        {
            UE_LOG(CancerInputLog, Verbose, TEXT("[%s] Action %s is interrupting abilities with tags %s."),
     ...
     */
    static void InterruptAbilityByInputID(const UCancerInputManagerComponent* Manager, const UInputAction* InputAction, const int32 InputID)
    {
        const TObjectPtr<UAbilitySystemComponent> AbilitySystemComponent = Manager->GetAbilitySystemComponent();
        if (ensure(IsValid(AbilitySystemComponent)))
        {
            UE_LOG(CancerInputLog, Verbose, TEXT("[%s] Action %s is interrupting abilities with ID %d."),
