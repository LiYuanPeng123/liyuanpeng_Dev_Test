...
TSharedRef<SWidget> SHTNDiff::GenerateDiffPanels()
{
	...
	ToolbarBuilder.AddToolBarButton(
		FUIAction(FExecuteAction::CreateSP(this, &SHTNDiff::OnToggleShowNodeMoves)),
		NAME_None,
		LOCTEXT("ShowNodeMovesLabel", "Show Node Moves"),
		LOCTEXT("ShowNodeMovesTooltip", "Show or hide differences that are just moving nodes around"),
		TAttribute<FSlateIcon>(this, &SHTNDiff::GetShowNodeMovesImage)
	);
...
void SHTNDiff::BuildDiffSourceArray()
{
	...
	if (!bShowNodeMoves)
	{
		FoundDiffs->RemoveAll([](const FDiffSingleResult& Diff)
		{
			return Diff.Diff == EDiffType::NODE_MOVED;
		});
	}
	...
	{
		DiffListSource.Add(MakeShared<FHTNDiffResultItem>(Diff));
	}
}
...
void SHTNDiff::OnToggleShowNodeMoves()
{
	bShowNodeMoves = !bShowNodeMoves;
	BuildDiffSourceArray();
	if (DiffList.IsValid())
	{
...
FSlateIcon SHTNDiff::GetShowNodeMovesImage() const
{
	return FSlateIcon(FHTNAppStyleGetAppStyleSetName(), 
		bShowNodeMoves ? "Icons.Visible" : "Icons.Hidden");
}
