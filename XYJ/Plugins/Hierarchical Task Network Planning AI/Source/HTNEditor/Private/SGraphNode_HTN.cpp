	...
	[
		...
		[
			...
			[
				...
				[
					SNew(SBox)
					.WidthOverride_Lambda([WeakNodeTitle = NodeTitle.ToWeakPtr()]() -> FOptionalSize
					{
					...
					.HeightOverride_Lambda([WeakNodeTitle = NodeTitle.ToWeakPtr()]() -> FOptionalSize 
					{
						const TSharedPtr<SNodeTitle> NodeTitlePin = WeakNodeTitle.Pin();
...
EVisibility SGraphNode_HTN::GetDescriptionVisibility() const
{
	EVisibility BaseVisibility = SGraphNodeAI::GetDescriptionVisibility();
	
	// Let's never collapse things we want hidden so that 
	// the size of the node doesn't change when we zoom out and the description gets hidden.
