...
void UHTNPlanInstance::OnTaskFinished(const UHTNTask* Task, uint8* TaskMemory, const FHTNPlanStepID& FinishedStepID, EHTNNodeResult Result)
{
	...
	// Apply worldstate changes of the finished step based on the task's configuration.
	...
	const EHTNTaskPlannedChangesApplicationTime Time = 
		Result == EHTNNodeResult::Succeeded ? EHTNTaskPlannedChangesApplicationTime::AfterExecutionSuccessful :
		Result == EHTNNodeResult::Aborted ? EHTNTaskPlannedChangesApplicationTime::AfterExecutionAborted :
		Result == EHTNNodeResult::Failed ? EHTNTaskPlannedChangesApplicationTime::AfterExecutionFailed :
		EHTNTaskPlannedChangesApplicationTime::Never;
	if (EnumHasAnyFlags(StaticCast<EHTNTaskPlannedChangesApplicationTime>(Task->WhenToApplyPlannedChanges), Time))
	{
...
bool UHTNPlanInstance::RecheckCurrentPlan(const FBlackboardWorldState* WorldStateOverride)
{
	...
	while (RecheckStack.Num())
	{
		...
		if (!bExecuting)
		{
			if (EnumHasAnyFlags(StaticCast<EHTNTaskPlannedChangesApplicationTime>(Task->WhenToApplyPlannedChanges), 
				EHTNTaskPlannedChangesApplicationTime::BeforeExecution))
			{
				CurrentStep.WorldState->ApplyChangedValues(*CurrentContext.WorldState);
			}

			if (!Task->WrappedRecheckPlan(*OwnerComponent, GetNodeMemory(CurrentStep.NodeMemoryOffset), 
				*CurrentContext.WorldState, CurrentStep))
			{
				UE_VLOG(this, LogHTN, Log,
		...
		// During recheck, we treat this as if the task had already executed successfully.
		if (EnumHasAnyFlags(StaticCast<EHTNTaskPlannedChangesApplicationTime>(Task->WhenToApplyPlannedChanges), 
...
EHTNNodeResult UHTNPlanInstance::StartExecuteTask(const FHTNPlanStepID& PlanStepID)
{
	...
	// Apply worldstate changes before execution if the task is configured to do so
	if (EnumHasAnyFlags(StaticCast<EHTNTaskPlannedChangesApplicationTime>(Task.WhenToApplyPlannedChanges), 
		EHTNTaskPlannedChangesApplicationTime::BeforeExecution))
	{
		PlanStep.WorldState->ApplyChangedValues(*OwnerComponent->GetBlackboardComponent());
	}
