...
namespace
{
	...
	FString TimingsToString(EHTNTaskPlannedChangesApplicationTime When)
	{
		if (When == EHTNTaskPlannedChangesApplicationTime::Never)
		{
			return TEXT("Does not apply planned changes");
		}
		
		TArray<FString, TInlineAllocator<4>> Timings;

		if (EnumHasAnyFlags(When, EHTNTaskPlannedChangesApplicationTime::BeforeExecution))
		{
			Timings.Add(TEXT("before execution"));
		}
		if (EnumHasAnyFlags(When, EHTNTaskPlannedChangesApplicationTime::AfterExecutionSuccessful))
		{ 
			Timings.Add(TEXT("after execution succeeded"));
		}
		if (EnumHasAnyFlags(When, EHTNTaskPlannedChangesApplicationTime::AfterExecutionAborted))
		{
			Timings.Add(TEXT("after execution aborted"));
		}
		if (EnumHasAnyFlags(When, EHTNTaskPlannedChangesApplicationTime::AfterExecutionFailed))
		{
			Timings.Add(TEXT("after execution failed"));
		}

		return FString::Printf(TEXT("Applies planned changes%s%s"),
			Timings.Num() <= 1 ? TEXT(" ") : TEXT("\n"),
			*FString::Join(Timings, TEXT(",\n")));
	}
}

UHTNTask::UHTNTask(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) {}

FString UHTNTask::GetStaticDescription() const
{
	if (StaticCast<EHTNTaskPlannedChangesApplicationTime>(WhenToApplyPlannedChanges) != 
		EHTNTaskPlannedChangesApplicationTime::AfterExecutionSuccessful)
	{
		return FString::Printf(TEXT("(%s)\n%s"), 
			*TimingsToString(StaticCast<EHTNTaskPlannedChangesApplicationTime>(WhenToApplyPlannedChanges)), 
...
void UHTNTask::WrappedLogToVisualLog(UHTNComponent& OwnerComp, uint8* NodeMemory, 
...
void UHTNTask::CreatePlanSteps(UHTNComponent& OwnerComp, UAITask_MakeHTNPlan& PlanningTask, 
