...
protected:
	...
	// For decorators placed on an If node, during execution (not planning) this returns true if the If node took the False branch.
	UFUNCTION(BlueprintPure, Category = "AI|HTN", Meta = (DisplayName = "Is On If Node False Branch"))
	FORCEINLINE bool IsOnIfNodeFalseBranch() const { return bIsOnIfNodeFalseBranch; }
	...
	uint8 bShowPropertyDetails : 1 = true;
	...
	mutable uint8 bIsOnIfNodeFalseBranch : 1 = false;

	// Set on node instances so we can pass it into functions that require it.
	mutable uint8* CachedNodeMemory = nullptr;
};