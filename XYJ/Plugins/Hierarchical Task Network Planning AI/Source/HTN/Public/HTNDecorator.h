...
public:
	...
	EHTNDecoratorTestResult WrappedTestCondition(UHTNComponent& OwnerComp, uint8* NodeMemory, 
		EHTNDecoratorConditionCheckType CheckType) const;
	EHTNDecoratorTestResult TestCondition(UHTNComponent& OwnerComp, uint8* NodeMemory, 
		EHTNDecoratorConditionCheckType CheckType) const;
	EHTNDecoratorTestResult GetLastEffectiveConditionValue(const uint8* NodeMemory) const;
	virtual bool ShouldCheckCondition(UHTNComponent& OwnerComp, uint8* NodeMemory, 
		EHTNDecoratorConditionCheckType CheckType) const;
...
protected:
	virtual void InitializeSpecialMemory(UHTNComponent& OwnerComp, uint8* NodeMemory, const FHTNPlan& Plan, 
		const FHTNPlanStepID& StepID) const;

	// Note that NodeMemory will be nullptr during plan-time checks, 
	// as memory blocks are only allocated once a plan is selected for execution.
	virtual bool CalculateRawConditionValue(UHTNComponent& OwnerComp, uint8* NodeMemory, 
		EHTNDecoratorConditionCheckType CheckType) const { return true; }
	...
	// Call this from a decorator that checks its condition in an event-based manner 
	// when it checks the condition in its event.
	// Takes the raw condition value, as returned from CalculateRawConditionValue.
	// Returns true if this resulted in an replan.
	bool NotifyEventBasedCondition(UHTNComponent& OwnerComp, uint8* NodeMemory, bool bRawConditionValue, 
		bool bCanAbortPlanInstantly = true) const;
	...
	struct FHTNDecoratorSpecialMemory : public FHTNNodeSpecialMemory
	{
		...
		// All decorators must turn to true for the False branch to be aborted, so caching the result of that 
		// evaluation prevents reevaluating all decorators on the If whenever one decorator notifies of its condition.
		//
	...
	// Whether the condition should be checked on tick. 
	// Note that how often it is checked is controlled by ConditionCheckInterval and not TickFunctionInterval.
	UPROPERTY(Category = Condition, EditAnywhere)
	uint8 bCheckConditionOnTick : 1;

	// If this and CheckConditionOnTick are true, 
	// the condition will only be checked on the first tick (when starting execution).
	...
	// During execution, how often should the condition be checked? 
	// Values less than or equal to zero will result in the condition being checked every tick.
	// Note that this is independent of TickFunctionInterval.
	UPROPERTY(EditAnywhere, Category = Condition, 
		Meta = (ForceUnits = s, EditCondition = "bCheckConditionOnTick && !bCheckConditionOnTickOnlyOnce"))
	...
	UPROPERTY(EditAnywhere, Category = Condition, Meta = (ClampMin = "0.0", ForceUnits = s), 
		Meta = (EditCondition = "bCheckConditionOnTick && !bCheckConditionOnTickOnlyOnce && ConditionCheckInterval > 0"))
	float ConditionCheckIntervalRandomDeviation;

	// How often should the Tick function be called during execution? 
	// Values less than or equal to zero will result in Tick being called every frame.
...
FORCEINLINE UWorldStateProxy* UHTNDecorator::GetWorldStateProxy(UHTNComponent& OwnerComp, 
	EHTNDecoratorConditionCheckType CheckType)
{
